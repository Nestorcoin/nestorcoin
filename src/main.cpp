...
#if defined(NDEBUG)
# error "Nestorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2da1ab849b6a6efb64dc85538e6c1f19cee3777682a07a32ec0bf2b8d2145121");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nestorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nestorcoin Signed Message:\n";
...
{
    // Nestorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nestorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nestorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1480226409, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Today will be a very good weather";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049f510bec2de1d96fd27b6c83aecf8de2a2f505931263ccbca331e71d58fb34eea4eae9b4c8976e6838dd20dc9b09930b07784f343086eaa6796d2babba1e3a24") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1480226409;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2198988;
        ...
        assert(block.hashMerkleRoot == uint256("0xb1281d06e7b5cef5b605c4c40fd01b12b3c6366b8e0131b8b94aa4d508851ccf"));
...
unsigned char pchMessageStart[4] = { 0x4d, 0xe5, 0x34, 0xc2 }; // Nestorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NestorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NestorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NestorcoinMiner(CWallet *pwallet)
{
    printf("NestorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nestorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NestorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NestorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NestorcoinMiner, pwallet));
}
